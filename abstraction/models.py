...
def gen_vecnorms(bin_year_by=MODEL_PERIOD_LEN, num_proc=1, model_dir=None,
    ...
    total_models = len(paths_df)
    periods = sorted(paths_df["period"].unique())
    print(f"Found {total_models} models across {len(periods)} periods: {', '.join(periods)}")

    word2field2z = defaultdict(dict)
    model_i = 0
    for period, pdf in sorted(paths_df.groupby("period")):
        vecnorm_rows = []
        corpus_groups = sorted(pdf.groupby(["corpus", "period_start", "period_end"]))
